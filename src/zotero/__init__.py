...
def _lower_title(title):
    ...
    for i in range(2):
        ...
        title = re.sub(
            r"([^-:!?]\s+(?:[A-Z][a-z]+(?:[-_]?[A-Z]?[a-z]+)*|[A-Z]{2}[a-z]{3,}))",
            lambda p: (
                g(p) if g(p)[1:].strip() in STATIC_WORDS else g(p)[0] + g(p)[1:].lower()
            ),
            title,
        )
...
class ZoteroCLI(object):
    def __init__(self, api_id=None, api_id_type="user", api_key=None, main_logger=None):
        ...
        for k in CACHE_FILES:
            ...
            if cached_file.exists():
                ...
                self._creds()
                self.__zot = self.__zot or zotero.Zotero(
                    CREDS_FILE.id, api_id_type, CREDS_FILE.secret
                )
                ...
                if k == "collections":
                    try:
                        ...
                        logger.warning(
                            "Beware to use the --group option if this is the ID of a group"
                        )
                        return
                elif k == "items":
    ...
    def _filter(self, fields=None, filters=None, force=False):
        ...
        for f in filters or []:
            ...
            if field in set(INTEGER_FIELDS) - set(["rank"]) and re.match(
                r"^(|<|>|<=|>=|==)\d+$", regex
            ):
                ...
                filt = (
                    not_,
                    OPERATORS[op],
                    lambda i, f: ZoteroCLI.date(i["data"][f], i),
                    ZoteroCLI.date(v),
                )
            elif field == "rank":
                ...
                filt = (
                    not_,
                    OPERATORS[op],
                    lambda i, f: self.ranks.get(i["key"], 0),
                    float(v),
                )
            # filter format: (negate, lambda, lambda's second arg)
                ...
                if regex not in self._valid_tags and regex not in ["-", "<empty>"]:
                    logger.warning(
                        f"Got tag '{regex}' ; should be one of:\n- "
                        + "\n- ".join(sorted(self._valid_tags, key=ZoteroCLI.sort))
                    )
                    raise ValueError("Tag '%s' does not exist" % regex)
                filt = (
                    not_,
                    lambda i, r: r in ["-", "<empty>"]
                    and i["data"]["tags"] in ["", []]
                    or r
                    in (
                        i["data"]["tags"].split(";")
                        if ts.is_str(i["data"]["tags"])
                        else [x["tag"] for x in i["data"]["tags"]]
                    ),
                    regex,
                )
            # filter format: (negate, lambda) ; lambda's second arg is the field name
                ...
                filt = (
                    (not_, lambda i, f: i["data"].get(f) == 1900)
                    if field == "year"
                    else (not_, lambda i, f: i["data"].get(f) == "")
                )
            else:
                filt = (
                    not_,
                    re.compile(regex, re.I),
                    lambda i, f: i["data"].get(f) or "",
                )
        ...
        for f in afields:
            if f not in self._valid_fields and regex != "-":
                logger.warning(
                    f"Got field name '{f}' ; should be one of:\n- "
        ...
        for i in self.items:
            ...
            if "authors" in afields:
                ...
                d["authors"] = [
                    c
                    for c in i["data"]["creators"]
                    if c["creatorType"] in ["author", "presenter"]
                ]
                    ...
                    for link in links:
                        ...
                        if "collections" in _filters.keys():
                            ...
                            for n, regex, _ in _filters["collections"]:
                                b = regex.search(
                                    ", ".join(
                                        self.__objects[x]["data"]["name"]
                                        for x in k["data"]["collections"]
                                    )
                                )
                                gb = gb and [b, not b][n]
                            if not gb:
                                continue
                        if ZoteroCLI.date(k["data"]["date"], k) > ZoteroCLI.date(
                            i["data"]["date"], i
                        ):
                            c += 1
                        if ZoteroCLI.date(k["data"]["date"], k) <= ZoteroCLI.date(
            ...
            if "collections" in afields:
                d["collections"] = [
                    self.__objects[k]["data"]["name"] for k in i["data"]["collections"]
                ]
            if "editors" in afields:
                d["editors"] = [
                    c for c in i["data"]["creators"] if c["creatorType"] == "editor"
                ]
            ...
            if "numAttachments" in afields:
                d["numAttachments"] = len(
                    [x for x in self.attachments if x["data"]["parentItem"] == i["key"]]
                )
            ...
            if "numEditors" in afields:
                d["numEditors"] = len(
                    [x for x in i["data"]["creators"] if x["creatorType"] == "editor"]
                )
            if "numNotes" in afields:
                d["numNotes"] = len(
                    [x for x in self.notes if x["data"]["parentItem"] == i["key"]]
                )
            if "numAnnotations" in afields:
                d["numAnnotations"] = len(
                    [x for x in self.annotations if x["data"]["parentItem"] == i["key"]]
                )
            ...
            for field, tfilters in _filters.items():
                for tfilter in tfilters:
                    if isinstance(tfilter[1], re.Pattern):
                        b = tfilter[1].search(
                            self._format_value(tfilter[2](tmp_i, field), field)
                        )
                ...
                if pass_item:
                    break
            if not pass_item and (
                not tmp_i["key"] in self.marks.get("ignore", []) or force
            ):
                yield tmp_i
    ...
    def _format_value(self, value, field=""):
        ...
        if field == "tags":
            ...
            return re.sub(
                r"([a-z0-9])([A-Z])", r"\1 \2", re.sub(r"(.)([A-Z][a-z]+)", r"\1 \2", v)
            ).lower()
        elif field == "rank":
            ...
            return [", ", ";"][field == "attachments"].join(
                self._format_value(x, field) for x in v
            )
        elif ts.is_dict(v):
            if field in ["authors", "creators", "editors", "firstAuthor"]:
                v = (
                    v.get("name")
                    or "{} {}".format(
                        v.get("lastName", ""), v.get("firstName", "")
                    ).strip()
                )
        return str(v)

    def _items(
        self, fields=None, filters=None, sort=None, desc=False, limit=None, force=False
    ):
        ...
        if (
            ...
            or lfield == "rank"
            or sort == "rank"
            or "rank" in [f.split(":")[0].lstrip("~") for f in filters or []]
        ):
        ...
        items = {
            ...
            for i in self._filter(
                ffields, [f for f in filters if not re.match(r"\~?rank\:", f)], force
            )
        }
        if len(items) == 0:
            logger.info("No data")
        ...
        if "rank" in ffields:
            ...
            self.ranks = {
                k: 1.0 / len(items) if i["data"]["year"] > 1900 else 0.0
            ...
            for n in range(
                len(self.ranks)
            ):  # at most N iterations are required (N being the number of keys)
                for k1 in self.ranks.keys():
                    ...
                    self.ranks[k1] = float(
                        len([None for l in links if l.split("/")[-1] in items.keys()])
                        > 0
                    )
                    for link in links:
                        k2 = link.split("/")[-1]
                        ...
                        if ZoteroCLI.date(k1_d["date"], k1_d) <= ZoteroCLI.date(
                            k2_d["date"], k2_d
                        ):
                            ...
                            if k2_d:
                                ...
                                if r > 0:
                                    ...
                                    self.ranks[k1] += self.ranks.get(k2, 0.0) / r
            ...
            if age:
                ...
                dt = set(
                    ZoteroCLI.date(i["data"]["date"]).timestamp()
                ...
                df_func = (
                    lambda dt: (float(ZoteroCLI.date(dt).timestamp() - dt_min) / ddt)
                    ** order
                )
                self.ranks = {
                    k: df_func(items[k]["data"]["date"]) * v
                    for k, v in self.ranks.items()
                }
            # finally, we normalize ranks
            max_rank = max(self.ranks.values())
            self.ranks = {
                k: v / max_rank if max_rank else 0.0 for k, v in self.ranks.items()
            }
        ...
        if limit is not None:
            if lfield is not None:
                select_items = sorted(
                    items.values(),
                    key=lambda i: ZoteroCLI.sort(i["data"][lfield], lfield),
                )
            else:
            ...
            logger.debug(
                f"Limiting to {limit} items (sorted based on {lfield or sort} in "
        ...
        if "rank" in ffields:
            for i in items.values():
                i["data"]["rank"] = self.ranks.get(i["key"], 0.0)
        # format the selected items as table data
        logger.debug(f"Sorting items based on {sort}...")
        for i in sorted(
            items.values(), key=lambda i: ZoteroCLI.sort(i["data"].get(sort, "-"), sort)
        ):
            row = [
                self._format_value(i["data"].get(f), f) if i["data"].get(f) else "-"
                for f in fields
            ]
            if len(row) > 1 and all(
                x in ".-" for x in row[1:]
            ):  # row[0] is the item's key ; shall never be "." or "-"
                continue
            data.append(row)
        if desc:
            data = data[::-1]
    ...
    def _marks(self, marker, filters=None, sort=None, desc=False, limit=None):
        ...
        if m1 is None:
            raise ValueError(
                "Bad marker (should be one of: {})".format(
                    "|".join(x for p in MARKERS for x in p[:2])
                )
            )
    ...
    def export(
        ...
        if output_format == "xlsx":
            ...
            if "{stars}" in line_format:
                ...
                mr, i_rank = 0.0, [h.lower() for h in headers].index("rank")
                for row in data:
                    r = row[i_rank]
            ...
            for row in data:
                ...
                if "link" in d.keys() and "link_with_abstract" in line_format:
                    if "AbstractNote" in headers:
                        ...
                        d["link_with_abstract"] = (
                            d["link"]
                            if d["abstractshortnote"] == "."
                            else "%s - %s" % (d["link"], d["abstractshortnote"])
                        )
                if "{emoji}" in line_format:
                    d["emoji"] = TYPE_EMOJIS.get(d["type"], TYPE_EMOJIS["default"])
                if "{stars}" in line_format:
                    r = float(d["rank"]) if d["rank"] != "-" else 0.0
                    ...
                    d["stars"] = (
                        ...
                        if r < 0.35
                            ...
                            s
                            if 0.35 <= r < 0.65
                            else 2 * s if 0.65 <= r < 0.85 else 3 * s
                        )
                    )
    ...
    def list(self, field, filters=None, desc=False, limit=None):
        ...
        data = [
            [x]
            for x in sorted(set(l), key=lambda x: ZoteroCLI.sort(x, field))
            if x != "-"
        ]
        if desc:
            data = data[::-1]
    ...
    def plot(self, name, filters=None):
        ...
        if name == "software-in-time":
            data = {}
            for i in self._filter(
                ["title", "date"], ["itemType:computerProgram"] + filters
            ):
            ...
            logger.debug(
                f"Got chart name '{name}' ; should be one of:\n- "
                + "\n- ".join(sorted(CHARTS))
            )
    ...
    def show(self, fields=None, filters=None, sort=None, desc=False, limit=None):
        ...
        if not output_key:
            fields = ["key"] + fields
        headers, data = self._items(fields, filters, sort, desc, limit)
        keys, data = [row[fields.index("key")] for row in data], [
            row[int(not output_key) :] for row in data
        ]
        if not output_key:
            headers = headers[1:]
        if len(headers) > 0:
            ...
            for key, row in zip(keys, table.table_data[2:]):
                if t_idx > 0:
                    row[t_idx] = "\n".join(
                        ts.txt2italic(l) if len(l) > 0 else ""
                        for l in row[t_idx].split("\n")
                    )
                if key not in self.marks.get("read", []):
                    for i, v in enumerate(row):
                        row[i] = "\n".join(
                            ts.txt2bold(l) if len(l) > 0 else "" for l in v.split("\n")
                        )
            print(table)
                ...
                if len(d) == 0:
                    ...
                    for k in rel:
                        print(
                            "- %s"
                            % ts.txt2italic(
                                self.__objects[k.split("/")[-1]]["data"]["title"]
                            )
                        )
                else:
                    print("{: <24}:\n".format(hb))
    ...
    def sort(value, field=None):
        ...
        if field.startswith("date") or field.endswith("Date"):
            ...
            s = re.sub(r"^\$", "s", re.sub(r"^\@", "a", s.lstrip())).lstrip(
                string.punctuation
            )
            return s
        else:
